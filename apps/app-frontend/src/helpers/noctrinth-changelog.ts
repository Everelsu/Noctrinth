...
export const NOCTRINTH_CHANGELOG: NoctrinthVersionEntry[] = [
	{
		...
		body: `### Added
- Two accent controls, under Settings → Appearance. Themes fix their accent at one colour, so the only choice was the theme itself. Intensity runs from 40% to 160% of what the theme defines and moves the colour's chroma — how far it sits from grey; brightness runs from 60% to 130% and moves its lightness, which is the difference between a lavender and a violet. Hue is never touched, so it stays the same colour throughout, and the whole app recolours while either slider moves. 100% on both is the theme untouched.
- The wordmark says when the app is busy. The launcher has always counted what it was waiting on, but the only thing reading that count was a progress bar it keeps switched off, so a page that took a moment looked like a click that had not registered. The knot now turns and a light in the accent colour sweeps across the lettering while something is in flight — held for one full turn, since most loads here finish faster than the eye does. It stays still for anyone who has asked for less motion.
