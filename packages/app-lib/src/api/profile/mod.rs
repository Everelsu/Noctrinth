...
///   - `datapacks/` (.zip) — globally-applied datapacks (some launcher
///     setups put per-world datapacks under `saves/<world>/datapacks/`; we
///     don't scan those — global only)
///
/// Without this, installing a shader/resource-pack on a vanilla instance
/// would appear to work (the file lands in the right folder) but the
/// installed-state badge would never refresh on tab return because the
/// fingerprint scanner couldn't see it.
///
...
pub async fn compute_cf_fingerprints(
    ...
    for (folder, expected_ext) in scan_targets {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            // Stream into a buffer. Most CF files are under 10 MiB; outliers
            // (huge shader packs, modpacks) still hash in well under a sec.
...
mod cf_fingerprint_tests {
    ...
    /// Murmur2 of empty input with seed=1 is a fixed, easy-to-derive value:
    /// h = 1, no body, no tail, h ^= h>>13 = 1, h *= M = 0x5bd1e995,
    /// h ^= h>>15 = 0xb7a3, → 0x5bd1e995 ^ 0xb7a3 = 0x5bd15e36.
    #[test]
    fn murmur2_empty_seed1() {
        assert_eq!(murmur2(&[], 1), 0x5BD1_5E36);
    }

    /// Whitespace stripping: a file containing only whitespace bytes hashes
    /// the same as the empty file.
    #[test]
    ...
    /// Known reference: "Hello World!" (no whitespace stripped — there's
    /// a single space inside that gets removed → effectively hashes
    /// "HelloWorld!"). The result is the standard Murmur2(seed=1) for those
    /// 11 bytes; computed against a verified reference implementation.
    #[test]
