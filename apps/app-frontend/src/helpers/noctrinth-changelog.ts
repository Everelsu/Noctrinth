...
export const NOCTRINTH_CHANGELOG: NoctrinthVersionEntry[] = [
	{
		...
		body: `### Added
- Accent presets, under Settings → Appearance, chosen the way a theme is. A theme fixes its accent at one colour, and the accent is most of where the interface's character comes from — so there are nine to pick from: the theme's own, Amethyst, Nightshade, Midnight, Glacier, Verdant, Lantern, Ember and Rose. Each carries two values, a deeper one for light themes and a brighter one for dark, and the whole app repaints the moment one is picked. The background goes with it, unless you would rather it did not: every surface the app is built out of takes the same hue at a fraction of the strength, keeping the lightness the theme gave it, so Ember is a warm dark app rather than a violet one with orange buttons. Four things never followed the accent at all, because the fork had written them out in purple by hand: the right-hand sidebar, the promo cards, the dimmer a dialog puts over the window, and the far end of the loading bar — which used to leave the accent behind halfway across and finish in lilac whatever colour it started. All four follow the preset now, and they do so whether or not the backgrounds are tinted, since they were the brand's to begin with. The toggle beside the presets governs the greys alone: turned off, the backgrounds and panels are the theme's own.
- The preset reaches the window itself. The splash screen comes up in it — its wash, its logo and its progress bar, all three of which had stayed purple because that screen carries a theme class of its own and so re-declared every colour on itself — and the icon in the title bar and the taskbar is redrawn in it too, the mark being a single shape that can simply be painted again. The choice is remembered outside the database as well, so the window opens in the right colour instead of correcting itself a moment later.
...
### Fixed
- Closing the app killed it with a panic instead of letting it exit. The skin flush that runs on the way out is given a deadline, and the timer for that deadline was built on the window's event loop — where there is no runtime to build one in. Tokio says so by panicking, which is what "there is no reactor running" was, and the process ended on exit code 101 every time.
- A machine with no DNS filled the log with faults. "No such host" arrives from Windows as a Winsock code Rust has no name for, so it slipped past the check that quiets the network's own failures and was written out as an error on every poll. It is a warning like the rest of them now, with tests over the codes it covers.
