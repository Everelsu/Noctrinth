...
export const NOCTRINTH_CHANGELOG: NoctrinthVersionEntry[] = [
	{
		...
		body: `### Added
- CurseForge modpack installation is back, rebuilt on the new 0.15 content pipeline: the instance is created from the pack manifest (Minecraft version + loader), every file is resolved and downloaded (with a CDN fallback for author-restricted files), overrides are applied, and Minecraft is installed.
...
- The embedded manual-download window no longer pops up on ordinary download failures; only author-restricted files use it.
