...
export const NOCTRINTH_CHANGELOG: NoctrinthVersionEntry[] = [
	{
		...
		body: `### Added
- An accent intensity, under Settings → Appearance. Themes fix their accent at one colour, so the only choice was the theme itself; the slider runs from 40% to 160% of what the theme defines, and the whole app recolours while it moves. It works on the colour's chroma alone — how far it sits from grey — leaving lightness and hue exactly where the theme put them, so nothing that depends on contrast shifts with it. 100% is the theme untouched.
- The wordmark says when the app is busy. The launcher has always counted what it was waiting on, but the only thing reading that count was a progress bar it keeps switched off, so a page that took a moment looked like a click that had not registered. A light in the accent colour now sweeps across the lettering while something is in flight, and holds still for anyone who has asked for less motion.
