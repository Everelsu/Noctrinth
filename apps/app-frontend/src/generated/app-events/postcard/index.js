...
const varint_max = (n_bytes) =>
	Math.floor((n_bytes * BITS_PER_BYTE + (BITS_PER_BYTE - 1)) / BITS_PER_VARINT_BYTE)
...
class Deserializer {
	...
	deserialize_string = () =>
		new TextDecoder().decode(new Uint8Array(this.pop_n(Number(this.try_take(U32_BYTES)))))
	deserialize_array = (des, len) =>
		Array.from({ length: len === undefined ? Number(this.try_take(U32_BYTES)) : len }, (v, i) =>
			des(this),
		)
		...
		case 4:
			return {
				...
				value: {
					...
					icon: d.deserialize_number(U32_BYTES, false) === 0 ? undefined : d.deserialize_string(),
					pack_id:
						d.deserialize_number(U32_BYTES, false) === 0 ? undefined : d.deserialize_string(),
					pack_version:
						d.deserialize_number(U32_BYTES, false) === 0 ? undefined : d.deserialize_string(),
				},
			}
...
function deserialize_LOADING_PAYLOAD(d) {
	return {
		...
		fraction:
			d.deserialize_number(U32_BYTES, false) === 0
				? undefined
				: d.deserialize_number_float(U64_BYTES),
...
function deserialize_INSTALL_JOB_SNAPSHOT(d) {
	return {
		...
		progress:
			d.deserialize_number(U32_BYTES, false) === 0 ? undefined : deserialize_INSTALL_PROGRESS(d),
		details: deserialize_INSTALL_PHASE_DETAILS(d),
		display:
			d.deserialize_number(U32_BYTES, false) === 0 ? undefined : deserialize_INSTALL_JOB_DISPLAY(d),
		error:
			d.deserialize_number(U32_BYTES, false) === 0 ? undefined : deserialize_INSTALL_ERROR_VIEW(d),
		rollback_error:
			d.deserialize_number(U32_BYTES, false) === 0 ? undefined : deserialize_INSTALL_ERROR_VIEW(d),
...
function deserialize_INSTALL_PROGRESS(d) {
	return {
		...
		secondary:
			d.deserialize_number(U32_BYTES, false) === 0
				? undefined
				: deserialize_INSTALL_PROGRESS_SECONDARY(d),
	}
}
...
function deserialize_INSTALL_ERROR_VIEW(d) {
	return {
		...
		phase:
			d.deserialize_number(U32_BYTES, false) === 0 ? undefined : deserialize_INSTALL_PHASE_ID(d),
		message: d.deserialize_string(),
		reason:
			d.deserialize_number(U32_BYTES, false) === 0
				? undefined
				: deserialize_SHARED_INSTANCE_UNAVAILABLE_REASON(d),
		api:
			d.deserialize_number(U32_BYTES, false) === 0
				? undefined
				: deserialize_INSTALL_API_ERROR_DETAILS(d),
		context:
			d.deserialize_number(U32_BYTES, false) === 0
				? undefined
				: deserialize_INSTALL_ERROR_CONTEXT(d),
	}
}
