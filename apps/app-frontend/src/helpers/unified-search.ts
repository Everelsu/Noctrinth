...
import { lookupByModrinthId, lookupByModrinthSlug, recordMapping } from './cross-platform-mapping'
...
export async function unifiedSearch(
	...
	for (const mrHit of mrHits) {
		...
		if (!cfMatch) {
			const mapped = lookupByModrinthId(mrHit.project_id) ?? lookupByModrinthSlug(mrHit.slug)
